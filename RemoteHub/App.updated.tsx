...
import { AuthService, DataService, UIComponents, StorageService } from './services/adapters/serviceAdapter';

// 可选的新组件导入（根据功能开关动态加载）
let DatabaseConfigModal: React.ComponentType<any> | null = null;
let LoadingButton: React.ComponentType<any> | null = null;
let LoadingSpinner: React.ComponentType<any> | null = null;

// 动态导入新组件
if (config.isDatabaseConfigModalEnabled()) {
  import('./components/DatabaseConfigModal').then(module => {
    DatabaseConfigModal = module.DatabaseConfigModal;
  });
}

if (config.isLoadingStatesEnabled()) {
  import('./components/LoadingStates').then(module => {
    LoadingButton = module.LoadingButton;
...
const AppContent: React.FC<AppContentProps> = ({ currentUser, onLogout }) => {
  ...
  const renderLoadingState = () => {
    if (!config.isLoadingStatesEnabled()) {
      // 原始的简单加载指示器
      return <div className="animate-spin w-8 h-8 border-4 border-blue-600 border-t-transparent rounded-full"></div>;
    }
  ...
  return (
    <div className="flex h-screen bg-slate-950 text-slate-100 font-sans selection:bg-blue-500/30">
      <Sidebar
        ...
        // 🔥 新增：数据库配置按钮
        onOpenDatabaseConfig={handleDatabaseConfig}
      />
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden relative bg-slate-950">
        <header className="h-16 border-b border-slate-800 flex items-center justify-between px-8 bg-slate-950/80 backdrop-blur-md z-10 sticky top-0">
          ...
          <div className="flex items-center gap-6">
            ...
            {/* 🔥 使用增强的加载按钮 */}
            {config.isLoadingStatesEnabled() && LoadingButton ? (
              <LoadingButton
        ...
        <div className="flex-1 overflow-y-auto p-8 custom-scrollbar scroll-smooth">
          {hosts.length === 0 && vpns.length === 0 && !isLoadingData ? (
      ...
      {/* 🔥 数据库配置模态框 */}
      {config.isDatabaseConfigModalEnabled() && DatabaseConfigModal && (
        <DatabaseConfigModal
...
const AuthGuard: React.FC = () => {
  ...
  const handleLoginSuccess = async () => {
    try {
      ...
      console.error('登录成功后获取用户信息失败:', error);
    }
  };
    ...
    if (config.isLoadingStatesEnabled()) {
      return (
        <div className="min-h-screen bg-slate-950 flex items-center justify-center">
          {LoadingSpinner && <LoadingSpinner />}
          <span className="ml-3 text-slate-400">初始化中...</span>
        </div>
      );
    }
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if (config.isDebugModeEnabled()) {
      console.log('[App] 当前功能开关状态:');
      console.log('Enhanced Auth:', config.isEnhancedAuthEnabled());
      console.log('Loading States:', config.isLoadingStatesEnabled());
      console.log('Database Config:', config.isDatabaseConfigModalEnabled());
      console.log('API Storage:', config.shouldUseApi());
      console.log('Debug Mode:', config.isDebugModeEnabled());
    }
  }, []);
