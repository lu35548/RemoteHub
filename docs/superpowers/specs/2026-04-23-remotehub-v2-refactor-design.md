...
> ⚠️ **DB 部分已被覆盖（2026-07-17）**：本文档基于 MySQL 体系（§2.4 MySQL 统一、§9.2 连接池、§9.6 加密等）。2026-07-17 起切换 SQLite + WAL，见 `docs/superpowers/specs/2026-07-17-v2-followup-design.md` §1。本文档 DB 相关章节（§2.4 数据库策略 / §9.2 连接池 / §6 部署的 db 容器 / §9.5 等）以该 spec 为准，**其余章节（认证/权限/CRUD/中间件/错误码/部署架构）仍有效**。一期代码已据此完成全修（commits 4af159e..e3a865b）+ 审计（`2026-06-24-remotehub-audit.md`）。
