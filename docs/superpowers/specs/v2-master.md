...
【v2 收尾】(2026-07-17 spec, 5 项 — phase2 的硬前置)
  ...
  ③ BLOCKER-2 CI
  ④ B-6 补核心 service 测试
  ⑤ 前端迁移（独立子项目，解锁 phase2 §19）✅ 完成（T1–T12，2026-08-31）
        ↓
...
> ✅ **前端迁移悬空已关闭（2026-08-31 T12 收官）**：⑤ 已于 2026-08-25 立项（`2026-08-25-frontend-migration.md`，父票 #1），T1–T12（#2–#13）全部完成，T11 浏览器级总验收通过，v1 参照目录 `RemoteHub/` 已删（`862a3f5`）。**phase2 §19 解锁**。原 meta-review 悬空警告（2026-07-18）自此失效。迁移期悬置的改良项/怪癖/工程债已集中收编至 `phase2-backlog.md`（开工输入）。
...
| phase2 | ❌ 0% 实施（待 v2 收尾） |
| 前端迁移 | ✅ 100%（T1–T12 全闭环 2026-08-31；总验收过，v1 目录已删 `862a3f5`） |
...
- **前端迁移**：phase2 硬前置，独立子项目（✅ 2026-08-31 完成）。
- **phase2 §19 前端**：✅ 已解锁（前端迁移完成）。
