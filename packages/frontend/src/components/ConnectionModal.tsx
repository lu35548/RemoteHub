...
import type {
  ConnectionDetail, ConnectionListItem, CreateConnectionRequest, ProjectListItem, Protocol, VpnType,
} from '@remotehub/shared';
import { PROTOCOL_SHORT_LABELS, PROPRIETARY_PROTOCOLS, WEB_PROTOCOLS, getProtocolColor } from '../constants';
...
interface ConnectionModalProps {
  isOpen: boolean;
  onClose: () => void;
  /** editingId 存在时 App 层走 update；返回 saved detail 供快速建 VPN 回填 id（v2 改造，v1 靠预生成 id） */
  onSave: (data: CreateConnectionRequest, editingId?: string) => Promise<ConnectionDetail>;
  projects: ProjectListItem[];
  connections: ConnectionListItem[];
  editingConnection: ConnectionDetail | null;
...
// 表单内部形态：port 是 input 字符串、tags 是数组（提交时转 API 形态，v2 tags 是逗号字符串）
...
const HOST_PROTOCOLS: { id: Protocol; name: string }[] = [
  { id: 'RDP', name: '桌面远程' },
  { id: 'SSH', name: 'SSH 终端' },
  { id: 'TODESK', name: 'ToDesk' },
  { id: 'SUNLOGIN', name: '向日葵' },
...
// v1 VPN_TYPES（登录方式视角）→ v2 枚举（协议类型视角）；顺序与默认项（OTHER=客户端软件）对齐 v1
const VPN_TYPE_OPTIONS: { id: VpnType; name: string; desc: string }[] = [
  { id: 'OTHER', name: '其他客户端', desc: 'FortiClient, EasyConnect' },
  { id: 'SSL_VPN', name: 'SSL VPN（网页认证）', desc: 'Web Portal / SSL VPN' },
  { id: 'OPENVPN', name: 'OpenVPN', desc: 'OpenVPN GUI / Config' },
  { id: 'WIREGUARD', name: 'WireGuard', desc: 'WireGuard Tunnel' },
  { id: 'IPSEC', name: 'IPsec / L2TP', desc: '系统原生 VPN' },
];

const createEmptyHost = (projectId?: string): ConnectionFormState => ({
  projectId: projectId || '',
  name: '',
  host: '',
  port: '',
...
const createEmptyVpn = (projectId?: string): ConnectionFormState => ({
  projectId: projectId || '',
  name: '',
  host: '',
  port: '',
...
const toFormState = (c: ConnectionDetail): ConnectionFormState => ({
  ...
  protocol: c.protocol,
  username: c.username ?? '',
  password: '', // v2 加密模型无明文可回填；留空 = 不提交 password（保持原密码）
  notes: c.notes ?? '',
  tags: c.tags ? c.tags.split(',').filter(Boolean) : [],
  requiredVpnId: c.requiredVpnId ?? '',
  vpnType: c.vpnType ?? undefined,
});
...
const ConnectionModal: React.FC<ConnectionModalProps> = ({
  ...
  // 编辑 host 时若关联了 VPN，VPN tab 回填该 VPN 数据并以 vpnEditingId 走 update（v1 联动行为）
  const [vpnEditingId, setVpnEditingId] = useState<string | null>(null);

  const [hostFormData, setHostFormData] = useState<ConnectionFormState>(createEmptyHost());
  const [vpnFormData, setVpnFormData] = useState<ConnectionFormState>(createEmptyVpn());
  const [tagInput, setTagInput] = useState('');
  // 快速建 VPN 刚保存的节点名：已关联提示优先用它（此时列表尚未 invalidate 刷新，v1 依赖父层刷新时序）
  const [savedVpnName, setSavedVpnName] = useState<string | null>(null);

  useEffect(() => {
    if (isOpen) {
      const defaultProjectId = editingConnection?.projectId ?? activeProjectId ?? projects[0]?.id ?? '';
      if (editingConnection) {
        if (editingConnection.protocol === 'VPN') {
          setMode('VPN');
          setVpnFormData(toFormState(editingConnection));
          setVpnEditingId(editingConnection.id);
          setHostFormData(createEmptyHost(defaultProjectId));
        } else {
          setMode('HOST');
          setHostFormData(toFormState(editingConnection));
          const linkedVpn = editingConnection.requiredVpnId
            ? connections.find(c => c.id === editingConnection.requiredVpnId)
            : undefined;
          if (linkedVpn) {
            setVpnFormData({
              projectId: linkedVpn.projectId,
              name: linkedVpn.name,
              host: linkedVpn.host,
              port: linkedVpn.port != null ? String(linkedVpn.port) : '',
              protocol: linkedVpn.protocol,
              username: linkedVpn.username ?? '',
              password: '',
              notes: '',
              tags: linkedVpn.tags ? linkedVpn.tags.split(',').filter(Boolean) : [],
              requiredVpnId: '',
              vpnType: linkedVpn.vpnType ?? undefined,
            });
            setVpnEditingId(linkedVpn.id);
          } else {
            setVpnFormData(createEmptyVpn(editingConnection.projectId));
            setVpnEditingId(null);
          }
        }
      } else {
        setMode('HOST');
      ...
      setSavedVpnName(null);
      setTagInput('');
    }
    // 初始数据只由打开/编辑目标/项目列表决定；connections 仅用于 linkedVpn 回填
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isOpen, editingConnection]);

  // modal 打开期间新建了项目时联动选中（v1 行为）
  useEffect(() => {
  ...
  const buildPayload = (data: ConnectionFormState, isEdit: boolean, isLinkedVpnEdit: boolean): CreateConnectionRequest => {
    ...
    // 编辑时密码留空 = 不动原密码（提交 null/'' 会清空）；新建时空 = null
    if (!isEdit || data.password) payload.password = data.password || null;
    // host 联动编辑关联 VPN 时，notes 来自列表项（无该字段），空 = 未回填而非用户清空 → 不提交以保留原值；
    // 直接编辑（detail 回填）时空 = 用户清空 → 正常提交 null
    if (!isLinkedVpnEdit || data.notes) payload.notes = data.notes || null;

    if (data.protocol === 'VPN') {
      payload.vpnType = data.vpnType ?? null;
      payload.requiredVpnId = null;
      // v1 VpnType.WEB 的 URL 自动补全逻辑，v2 语义映射到 SSL_VPN（网页认证）；vpnLoginUrl 由 host 推导，无独立输入
      if (data.vpnType === 'SSL_VPN') {
        let url = data.host.trim();
        if (!/^https?:\/\//i.test(url)) url = `https://${url}`;
        payload.host = url;
        payload.vpnLoginUrl = url;
      } else {
        payload.vpnLoginUrl = null;
      }
    } else {
      payload.vpnType = null;
      payload.vpnLoginUrl = null;
      payload.requiredVpnId = data.requiredVpnId || null;
    }
    return payload;
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'VPN' && isCreatingVpnForHost) {
        // 快速建 VPN：保存后按后端返回的 detail 回填 id（v2 改造，v1 靠预生成 id）
        const saved = await onSave(buildPayload(currentData, false, false));
        setHostFormData(prev => ({ ...prev, requiredVpnId: saved.id }));
        setSavedVpnName(saved.name);
        setMode('HOST');
        setIsCreatingVpnForHost(false);
        setVpnFormData(createEmptyVpn(hostFormData.projectId));
        setVpnEditingId(null);
      } else {
        const editingId = mode === 'VPN' ? vpnEditingId ?? editingConnection?.id : editingConnection?.id;
        // host 联动编辑关联 VPN：vpnEditingId 指向 linkedVpn 而非 editingConnection（review S2 修复）
        const isLinkedVpnEdit = mode === 'VPN' && vpnEditingId != null && vpnEditingId !== editingConnection?.id;
        await onSave(buildPayload(currentData, editingId != null, isLinkedVpnEdit), editingId);
        onClose();
      }
    } catch (error) {
      console.error(error);
      // 错误已在 App 层 toast 提示，modal 保持打开
    } finally {
      setIsSaving(false);
    }
  };
  ...
  // 编辑既有连接时锁定所属项目：backend updateConnection 白名单不含 projectId（v2 项目是权限边界，
  // 跨项目移动需专门权限设计，归 phase2——ADR-0001 auth 例外同款处理，显式 Deviation 记 notes）
  const isEditingExisting = !!editingConnection || !!vpnEditingId;
  const isProprietary = PROPRIETARY_PROTOCOLS.includes(hostFormData.protocol || 'RDP');
  const isWeb = WEB_PROTOCOLS.includes(hostFormData.protocol || 'RDP');
  const isVpnWeb = mode === 'VPN' && currentData.vpnType === 'SSL_VPN';
  const vpnOptions = connections.filter(c => c.protocol === 'VPN' && c.id !== editingConnection?.id);
            ...
            <div className="col-span-12 lg:col-span-8 space-y-6 min-h-[480px]">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5 auto-rows-max">
                ...
                <div className="space-y-2">
                  ...
                  <select name="projectId" value={currentData.projectId || ''} onChange={handleChange} disabled={isSaving || isEditingExisting} title={isEditingExisting ? 'v2 暂不支持移动项目（规划中）' : undefined} className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2.5 text-sm text-white focus:ring-2 focus:ring-blue-500 outline-none appearance-none disabled:opacity-50">
                    {projects.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                  </select>
                </div>
                <div className="col-span-1 md:col-span-2 space-y-2">
                  <label className="text-sm font-medium text-slate-300 flex items-center gap-2">{isProprietary ? '设备代码 / 识别码' : isWeb ? '网址 URL' : isVpnWeb ? '登录地址 (URL)' : '主机地址 (IP / Domain)'} <span className="text-rose-400">*</span></label>
                  <div className="flex gap-3">
                    <input name="host" value={currentData.host || ''} onChange={handleChange} disabled={isSaving} placeholder={isProprietary ? '例如: 882 331 992' : isWeb || isVpnWeb ? 'https://...' : '192.168.1.100'} required className="flex-1 bg-slate-900 border border-slate-700 rounded-lg px-3 py-2.5 text-sm font-mono text-white focus:ring-2 focus:ring-blue-500 outline-none disabled:opacity-50" />
                    {!isProprietary && !isWeb && mode === 'HOST' && (
                      <input name="port" type="number" value={currentData.port || ''} onChange={handleChange} disabled={isSaving} placeholder={hostFormData.protocol === 'SSH' ? '22' : hostFormData.protocol === 'RDP' ? '3389' : 'Port'} className="w-24 bg-slate-900 border border-slate-700 rounded-lg px-3 py-2.5 text-sm font-mono text-white focus:ring-2 focus:ring-blue-500 outline-none disabled:opacity-50" />
                    )}
                  </div>
                </div>
                <div className="col-span-1 md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-5">
                  {currentData.protocol !== 'HTTP' ? (
                    <>
              ...
              {mode === 'HOST' && (
                <div className="bg-slate-900/50 rounded-xl p-4 border border-slate-800 mt-2">
                  ...
                  {hostFormData.requiredVpnId && <div className="mt-2 text-[10px] text-indigo-300 flex items-center gap-1"><Check size={10} /> 已关联: {savedVpnName ?? connections.find(c => c.id === hostFormData.requiredVpnId)?.name}</div>}
                </div>
              )}
            </div>
            <div className="col-span-12 lg:col-span-4 space-y-6 lg:border-l lg:border-slate-800 lg:pl-8">
              ...
              <div className="bg-slate-900/50 rounded-lg p-4 border border-slate-800">
                ...
                <div className="text-xs text-slate-400 space-y-1">
                  <div className="flex justify-between"><span>类型:</span><span className="text-slate-200">{PROTOCOL_SHORT_LABELS[currentData.protocol]}</span></div>
